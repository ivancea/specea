...
11. When a canonical path changes or a requirement is renamed, renumbered, or removed, update every repository reference in the same change and never reuse a removed identifier for different behavior.
12. For an explicitly requested spec move or rename, keep `spec.md` as the root filename, require that the target file does not already exist under `.specea/specs`, derive the new ID from the destination, and update its frontmatter and every reference.
...
- Verify every affected spec's frontmatter ID exactly matches its directory path.
